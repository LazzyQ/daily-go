	...
	var zipkinTracer *zipkin.Tracer
	{
		if *zipkinURL != "" {
			var (
				err error
				hostPort = ""
				serviceName = "addsrv-cli"
				reportor = zipkinhttp.NewReporter(*zipkinURL)
			)
			defer reportor.Close()
			zEP, _ := zipkin.NewEndpoint(serviceName, hostPort)
			zipkinTracer, err = zipkin.NewTracer(reportor, zipkin.WithLocalEndpoint(zEP))
			if err  != nil {
